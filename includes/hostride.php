<div class="modal fade" tabindex="-1" id="hostridemodel"
     ...
     xmlns:data="http://www.w3.org/1999/xhtml" xmlns:data="http://www.w3.org/1999/xhtml"
                        ...
                        <div class="row">
                            <div class="col-lg-8 offset-lg-2 text-xs-center">
                                <form name="hostride" id="hostride" action="/requests/hostriderequest.php" method="post">
                                    ...
                                    <div class="row control-group">
                                        <div class="form-group col-xs-12 floating-label-form-group controls">
                                            <label for="departureTime"></label><input type="time" class="form-control" data-placeholder="Departure Time" id="departureTime" name="departureTime" required data-validation-required-message="Please enter your time of travel." aria-required="true">
                                        </div>
                                    </div>
                                    <div class="row control-group">
                                        <div class="form-group col-xs-12 floating-label-form-group controls">
                                        <input type="text" class="form-control" placeholder="Vehicle Model/Type" id="vehicleModel" name="vehicleModel"  required data-validation-required-message="Please enter type of vehicle or model number." aria-required="true" maxlength="50" autocomplete="off" style="width: 550px;">
                                    </div>
                                    </div>
                                    <div class="row control-group">
                                        <div class="form-group col-xs-12 floating-label-form-group controls">
                                    ...
                                    <div class="row">
                                        <div class="form-group col-lg-4 offset-lg-4 floating-label-form-group controls text-xs-center">
                                            <input type="submit" class="btn btn-default font-white btn-back signup_btn text-lg-center" id="hostrideadded" name="hostrideadded" value="Host a ride">
                                        </div>
                                    </div>
                                    </form>
                                    <div class="row control-group">
...
<script>
    ...
    $(document).ready(function() {
        ...
        $("#vehicleModel").easyAutocomplete(options);
    });
        ...
        function callback(response, status) {
            if (status != google.maps.DistanceMatrixStatus.OK) {
                ...
                if (response.rows[0].elements[0].status === "ZERO_RESULTS") {
                    ...
                    var miles = distance_text.replaceAll("," , "" , true);
                    //var miles = distance_text.substring(0, distance_text.length - 3);
                    miles= miles.replaceAll("mi","", true);
                    var pay = miles* 0.4;

                    $('#hostrideresult').html("It is " + distance.text + " miles from " + origin + " to " + destination + "<br/>Total earning per passenger is: <strong>$" + pay +"</strong>"); //var distance = response.rows[0].elements[0].distance;
                   // var distance_value = distance.value;
                   // var distance_text = distance.text;
                   // var miles_text= "" + distance_text;

                  //  distance_text= distance_text.replaceAll("," ,"", true);
                  // distance_text= distance_text.replaceAll("," ,"", true);
                   // var miles = distance_text.substring(0, distance_text.length - 3);
                  // var miles_num =
                   // var pay = distance_text*0.1;
                   // $('#hostrideresult').html("It is " + distance.text + " miles from " + origin + " to " + destination + "<br/>Total earning per passenger is: <strong>$" + pay +"</strong>");
                }
            }
        }
