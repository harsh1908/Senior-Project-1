...
function getmyrequestedResults(){
    global $conn;

    $query = "SELECT * FROM requestedrides Where (email = '".$_SESSION['email']."')";
    $result = mysqli_query($conn, $query);
    if(mysqli_num_rows($result) > 0)
    {
        return $result;
...
function getmyhostedResults(){
    global $conn;

    $query = "SELECT * FROM hostedrides Where (email = '".$_SESSION['email']."')";
    $result = mysqli_query($conn, $query);
    if(mysqli_num_rows($result) > 0)
    {
        return $result;
...
function sendEmailVerification($email,$hash){
    ...
    $subject = 'Signup Verification | Moove'; // Give the email a subject
    $message = '
 
Thanks for signing up to Moove!
Your account has been created, you can login and start using Moove after you have activated your account by pressing the url below.
...
http://www.Moove.ml/requests/verify.php?email='.$email.'&hash='.$hash.'

';

    $headers = 'From:f6team8@moove.ml' . "\r\n"; // Set from headers
...
function sendShareRideEmail($email,$origin,$destination,$date,$time){
    ...
    $to      = $email; // Send email to our user
    $subject = 'Ride Share Request | Moove'; // Give the email a subject
    $message = '
 
You have one ride share request from '.$email.' on Moove for the ride from '.$origin.' to '.$destination.' on date '.$date.' at '.$time.'. 

As soon as we get email from your passenger that ride is completed Moove will release your payment.
';
...
function sendNewEmailVerification($email,$hash){
    ...
    $to      = $email; // Send email to our user
    $subject = 'Email Change Verification | Moove'; // Give the email a subject
    $message = '
 
You can change the email of your Moove account by pressing the url below.


Please click this link to change your email:
http://www.Moove.ml/requests/verify.php?newemail='.$email.'&hash='.$hash.'
...
function sendPasswordEmail($email,$password){
    $to      = $email; // Send email to our user
    $subject = 'Password Reset | Moove'; // Give the email a subject
    $message = '
 
Use the credentials given below to login to your Moove account.
...
function sendContactInfo($name, $email, $contactmessage){
    $to      = 'harshdesai7376@gmail.com'; // Send email to our user
    $subject = 'Contact Us Feedback | Moove'; // Give the email a subject
    $message = $name;
